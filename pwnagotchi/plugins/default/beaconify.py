...
class Beaconify(plugins.Plugin):
    ...
    def on_peer_lost(self, agent, peer):
        # The unit hears its own beacons.
        ...
        if peer.identity() == agent.fingerprint():
            if self.found_self:
                logging.warning("Can't hear my own beacons! Restarting pwngrid!")
                self.found_self = False
    ...
    def on_config_changed(self, config):
        ...
        if self.iface is None:
            self.iface = "wlan0mon"
            logging.info(f"[Beaconify] iface not defined in config. Setting to default {self.iface}")
        self.sleeptime = self.options.get('sleeptime')
        if self.sleeptime is None:
            self.sleeptime = 5
            logging.info(f"[Beaconify] sleeptime not defined in config. Setting to default {self.sleeptime}")
    ...
    def send_beacon(self, time_duration):
        def inner_func(obj, time_duration):
        ...
        Thread(target=inner_func, args=(self,time_duration)).start()
