...
class hashie(plugins.Plugin):
    ...
    __description__ = '''
                        ...
                        Modified by Artur Oliveira to remove "lonely pcaps".
                        '''
    
    def __init__(self):
        logging.info("[hashie-ng] plugin loaded")
        self.lock = Lock()
    ...
    def on_config_changed(self, config):
        ...
        if 'interval' not in self.options or not (self.status.newer_then_hours(self.options['interval'])):
            logging.info('[hashie-ng] Starting batch conversion of pcap files')
            with self.lock:
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        with self.lock:
            ...
            if handshake_status:
                logging.info('[hashie-ng] Good news:\n\t' + '\n\t'.join(handshake_status))
    ...
    def _writeEAPOL(self, fullpath):
        ...
        if os.path.isfile(fullpathNoExt +  '.22000'):
            logging.debug('[hashie-ng] [+] EAPOL Success: {}.22000 created'.format(filename))
            return True
        else:
            return False
    ...
    def _writePMKID(self, fullpath, apJSON):
        ...
        if os.path.isfile(fullpathNoExt + '.16800'):
            logging.debug('[hashie-ng] [+] PMKID Success: {}.16800 created'.format(filename))
            return True
        else: #make a raw dump
            result = subprocess.getoutput('hcxpcapngtool -K {}.16800 {} >/dev/null 2>&1'.format(fullpathNoExt,fullpath))
            if os.path.isfile(fullpathNoExt + '.16800'):
                if self._repairPMKID(fullpath, apJSON) == False:
                    logging.debug('[hashie-ng] [-] PMKID Fail: {}.16800 could not be repaired'.format(filename))
                    return False
                else:
                    logging.debug('[hashie-ng] [+] PMKID Success: {}.16800 repaired'.format(filename))
                    return True
            else:
                logging.debug('[hashie-ng] [-] Could not attempt repair of {} as no raw PMKID file was created'.format(filename))
                return False
    ...
    def _repairPMKID(self, fullpath, apJSON):
        ...
        fullpathNoExt = fullpath.split('.')[0]
        filename = fullpath.split('/')[-1:][0].split('.')[0]
        logging.debug('[hashie-ng] Repairing {}'.format(filename))
                ...
                if line.split(':')[0] == hashString.split(':')[1]: #if the AP MAC pulled from the JSON or tcpdump output matches the AP MAC in the raw 16800 output
                    ...
                    if (len(hashString.split(':')) == 4) and not (hashString.endswith(':')):
                        with open(fullpath.split('.')[0] + '.16800','w') as tempFileC:
                            logging.debug('[hashie-ng] Repaired: {} ({})'.format(filename,hashString))
                            tempFileC.write(hashString + '\n')
                        return True
                    else:
                        logging.debug('[hashie-ng] Discarded: {} {}'.format(line, hashString))
        else:
    ...
    def _process_stale_pcaps(self, handshake_dir):
        ...
        for num, handshake in enumerate(handshakes_list):
            ...
            done22000 = os.path.isfile(fullpathNoExt + '.22000')
            done16800 = os.path.isfile(fullpathNoExt + '.16800')
            if done22000 or done16800: #Ignore completed handshakes
                continue
            if not done22000: #if no 22000, try
                if self._writeEAPOL(handshake):
                    successful_jobs.append('22000: ' + pcapFileName)
                else:
                    failed_jobs.append('22000: ' + pcapFileName)
            if not done16800: #if no 16800, try
                if self._writePMKID(handshake, ""):
                    ...
                    if not os.path.isfile(fullpathNoExt + '.22000'): #if no 16800 AND no 22000
                        ...
                        logging.debug('[hashie-ng] Batch job: added {} to lonely list'.format(pcapFileName))
            if ((num + 1) % 50 == 0) or (num + 1 == len(handshakes_list)): #report progress every 50, or when done
                logging.info('[hashie-ng] Batch job: {}/{} done ({} fails)'.format(num + 1,len(handshakes_list),len(lonely_pcaps)))
        if successful_jobs:
            logging.info('[hashie-ng] Batch job: {} new handshake files created'.format(len(successful_jobs)))
        if lonely_pcaps:
            logging.info('[hashie-ng] Batch job: {} networks without enough packets to create a hash'.format(len(lonely_pcaps)))
            self._getLocations(lonely_pcaps)
        if len(lonely_pcaps) > 0:
            logging.info(f'[hashie-ng] {len(lonely_pcaps)} lonely (failed) handshakes will be deleted.')
            for filename in lonely_pcaps:
                pcapFileName = filename.split('/')[-1:][0]
                logging.info('[hashie-ng] The pcap file is not a valid handshake. Deleting file:' + pcapFileName.split('/')[01)
				os.remove(filename)
				# Confirm the pcap file was deleted.
				if not os.path.exists(filename):
					logging.debug('[hashie-ng] The pcap file was deleted for being incomplete. FILE: ' + pcapFileName)
				# If the pcap file was not deleted, then send an error to the log.
				if os.path.exists(filename):
					logging.error('[hashie-ng] Could not delete the pcap file. Please delete it manually. FILE: ' + pcapFileName)
        ...
        with open('/root/.incompletePcaps','w') as isIncomplete:
            ...
            for pcapFile in lonely_pcaps:
                ...
                if os.path.isfile(fullpathNoExt +  '.gps.json') or os.path.isfile(fullpathNoExt +  '.geo.json') or os.path.isfile(fullpathNoExt +  '.paw-gps.json'):
                    count +=1
            if count != 0:
                logging.info('[hashie-ng] Used {} GPS/GEO/PAW-GPS files to find lonely networks, go check webgpsmap! ;)'.format(str(count)))
            else:
                logging.info('[hashie-ng] Could not find any GPS/GEO/PAW-GPS files for the lonely networks'.format(str(count)))
        ...
        if locations:
            with open('/root/locations.csv','w') as tempFileD:
                for loc in locations:
                    tempFileD.write(loc + '\n')
            logging.info('[hashie-ng] Used {} GPS/GEO files to find lonely networks, load /root/locations.csv into a mapping app and go say hi!'.format(len(locations)))
